...
int solve()
{
    ...
    for(auto it:s)
        if(previous.count(it))
            answer+=previous.size()-1,previous.clear();

        else previous.insert(it);

    int totalVal=answer+previous.size();
    cout<<totalVal<<nl;
}
