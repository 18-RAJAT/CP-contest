...
void solve()
{
    ...
    cf(i,1,n)
    {
        // cin>>p.first;p.second=i+1;
        // if(p.first)pq.push(p);
        // int capacity;cin>>capacity;
        // if(capacity>0)pq.push({capacity,i});
        cin>>a;if(a>0)pq.push({a,i});
    }
        VPII answer;
        while(pq.size()>=2)
        {
            ...
            answer.PB(make_pair(p1.second,p2.second));
            p1.first--;p2.first--;
        ...
        cout<<answer.size()<<nl;
        f1(i,0,answer.size())
        {cout<<answer[i].first<<" "<<answer[i].second<<nl;}
...
// freopen("Error.txt","w",stderr); //Error detect
#endif
