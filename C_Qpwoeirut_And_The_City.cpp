...
int solve() {
    ...
    f1(i,0,n)cin>>height[i];
    f1(i,2,n) if(i%2==0)
    dp[i]=dp[i-2]+max(max(height[i-2]-height[i-1],height[i]-height[i-1])+1,0);
    else
    dp[i]=min(dp[i-1],dp[i-2]+max(max(height[i-2]-height[i-1],height[i]-height[i-1])+1,0));
cout<<dp[n-1]<<nl;
}
