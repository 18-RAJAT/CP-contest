...
void solve()
{
    ...
    while(l<=r)
    {
        ll req=a[l].second-have;
        //maximise the discount
        req=max(0LL,req);
        ll take=min(a[r].first,req);
        // price+=take*a[l].second;
        a[r].first-=take;
        price+=2*take;//take to buy and take to sell
        have+=take;

        if(a[r].first==0)
            //if we have to buy more
        {
            r--;
        }
        else
