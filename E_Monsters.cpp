...
void solve()
{
    ...
    f1(i,1,n+1)
    {
        if(visited[i]==0 and weights[i]==0)
        {
            set<PII>st;
            st.INS(make_pair(0,i));

            set<int>current;
            while(not st.empty())
            {
                auto[it,node]=*st.begin();
                st.erase(st.begin());
                visited[node]=true;
                ...
                {
                    // if(visited[child]==0)
                    if(current.find(child)==current.end())
                    {
                        // st.INS(make_pair(it+1,child));
                        st.INS(make_pair(weights[child],child));
                    }
                }
                // if(sza(current)>it)
                if(sza(current)==n)
