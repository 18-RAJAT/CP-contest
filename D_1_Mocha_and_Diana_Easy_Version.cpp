...
void solve()
{
    ...
    int parent1[n+1];MEM(parent1,0);
    int parent2[n+1];MEM(parent2,0);
    ...
    function<int(int[],int)>find([&](int parent[],int i)->int
    {
        if(parent[i]==i)
        {
            return i;
    ...
    auto dsu=[&](int parent[],int x,int y)->int
    {
        int a=find(parent,x);
        int b=find(parent,y);
        if(a!=b)
    ...
    f1(i,0,m1)
    {
        int x,y;
        cin>>x>>y;
        parent1[y]=find(parent1,x);
    }
    f1(i,0,m2)
    {
        int x,y;
        cin>>x>>y;
        parent2[y]=find(parent2,y);
    }
    int minimum=min(n-1-m1,n-1-m2);
    if(minimum==0)
    {
        cout<<0<<nl;
        return;
    ...
    cf(i,1,n)
    {
        cf(j,i,n)
        {
            if(find(parent1,i)!=find(parent1,j) and find(parent2,i)!=find(parent2,j))
            {
                cout<<i+1<<" "<<j+1<<nl;
                dsu(parent1,i,j);
                dsu(parent2,i,j);
