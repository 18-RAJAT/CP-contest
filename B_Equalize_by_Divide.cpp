...
void solve()
{
    ...
    vector<pii>ans;
    // --st.end();
    if(st.begin()->ff==(--st.end())->ff)
    {
        cout<<0<<nl;
        return;
    }

    while(st.begin()->ff!=(--st.end())->ff)
    {
        int db=st.begin()->ff;
        // auto it=st.begin();
        // auto it2=--st.end();
        // ans.push_back(make_pair(it->ss,it2->ss));
        int db1=st.begin()->ss;
        // st.erase(it);
        // st.erase(it2);
        // st.insert(make_pair(it->ff+1,it->ss));
        // st.insert(make_pair(it2->ff-1,it2->ss));
        auto it=*st.upper_bound({st.begin()->ff+1,0});
        // ans.push_back(make_pair(db1,it.ss));
        st.erase(it);

        // st.INS({it.ff-1,it.ss});
        st.INS(make_pair((it.ff-1)/db+1,it.ss));
        ans.pb(make_pair(db1,it.ss));
    }
    cout<<sza(ans)<<nl;
    for(auto it:ans)
    {
