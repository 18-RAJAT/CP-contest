...
void solve()
{
    ll n;cin>>n;ll result=0;
    priority_queue<int>a,b;
    // f1(i,0,n){cin>>x;p.push(x);}
    // f1(i,0,n){cin>>x;q.push(x);}
    // while(!p.empty())
    // {
    //     if(p.top()>q.top())
    ...
    //     int y=q.top();
    //     result++;
    //     if(x==y){p.pop();q.pop();result--;}
    //     else if(x>y){p.pop();p.push(to_string(x).length());}
    //     else{q.pop();q.push(to_string(y).length());}
    // }cout<<result<<nl;

        cf(i,1,n){ll x;cin>>x;a.push(x);}
        cf(i,1,n){ll x;cin>>x;b.push(x);}
        ...
        while(!a.empty())
        {
            ...
            if(x<y)
            {
                result++;b.pop();b.push(check(y));
            }
            else if(x>y)
            {
                result++;a.pop();a.push(check(x));
            }
            else
            {
                a.pop();b.pop();
