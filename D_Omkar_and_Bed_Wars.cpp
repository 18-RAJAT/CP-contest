...
void solve()
{
    int n;
        ...
        f1(i,0,4)
        {
            ...
            f1(i,3,n)
            {
                int m1=min(dp[i-1][0][a[i]],dp[i-1][1][a[i]]);
                // cout<<m1;
                int m2=min(dp[i-1][0][1-a[i]],dp[i-1][1][1-a[i]]);
                // cout<<m2;
                dp[i][0][a[i]]=min(m2,m1+1);
                dp[i][1][a[i]]=min(dp[i][0][a[i]],dp[i-1][0][a[i]]);
                dp[i][0][1-a[i]]=1+min(m1,m2+1);
                dp[i][1][1-a[i]]=1+min(dp[i][0][1-a[i]]-1,dp[i-1][0][1-a[i]]);
            }
            //cout<<dp[n-1][0]<<" "<<dp[n-1][1]<<nl;
            if(n==3)
            ans=min(ans, cur+min({dp[2][0][1], dp[2][0][0], dp[2][1][0], dp[2][1][1]}));
            else
            {
                int m1=min(dp[n-1][0][a[0]],dp[n-1][1][a[0]]);
                int m2=min(dp[n-1][0][1-a[0]],dp[n-1][1][1-a[0]]);
                if(a[0]==a[1])
                {
                    ans=min(ans,cur+(min(m2,m1+1)));
                    // cout<<ans;
                }
                else
                {
                    ans=min(ans,cur+min({m2,m1+1,dp[n-1][0][a[0]]}));
                    // cout<<ans;
                }
            }
        }
