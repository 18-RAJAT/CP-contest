...
void solve()
{
    ...
    for(auto it:mp)
    {
        colorPairs+=it.second/2;
        noMatchingPairs+=it.second%2;
    }

    // noMatchingPairs+=colorPairs;
    noMatchingPairs+=(colorPairs%m)*2;
    noMatchingPairs=min(noMatchingPairs,n);

    // int ans=(noMatchingPairs/m)*2;
