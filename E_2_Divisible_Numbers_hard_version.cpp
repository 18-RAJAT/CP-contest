...
bool solve()
{
    ...
    VPII afactors,bfactors;
    cf(i,1,sqrt(a))
    // for(int i=1;i*i<=a;++i)
    {
        if(a%i==0)
        {
            afactors.pb(make_pair(i,a/i));
            afactors.pb(make_pair(a/i,i));
        }
    }
       cf(i,1,sqrt(b))
    // for(int i=1;i*i<=b;++i)
    {
        if(b%i==0)
        {
            bfactors.pb(make_pair(i,b/i));
            bfactors.pb(make_pair(b/i,i));
        }
    }
    for(auto ait:afactors)
    {
        for(auto bit:bfactors)
        {
            ll a1=ait.first;
            ll a2=ait.second;
