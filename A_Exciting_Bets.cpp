...
int solve()
{
    // int n,m;cin>>n>>m;int temp=0;
    // VI a(n);
    // f1(i,0,n){cin>>a[i];temp++;}

    // VI s(n),e(n);
    // f1(i,0,n){s[i]=1;e[i]=n+5;temp--;}

    // f1(i,1,n){s[i]=i;if(a[temp]>a[temp]){e[i]=i;}else{e[temp]=i;temp=i;}}

    // cf(j,1,m)
    // {
    //     int i,r;cin>>i>>r;--i;
    //     if(e[i]==n+10){if(r>=s[i]){ll x=r;cout<<(x-s[i]+1)<<nl;}else{cout<<"0"<<nl;}}
    //     else{if(r>=s[i]){int x=min(r,e[i]-1);cout<<(x-s[i]+1)<<nl;}else{cout<<"0"<<nl;}}
    // }
    ll a,b;cin>>a>>b;
    ll va=abs(a-b);cout<<va<<" ";
    if(!va)cout<<"0"<<nl;
    else cout<<min(a%va,va-a%va)<<nl;
}
