...
void solve()
{
    function<ll(ll,ll,ll)>permute=[&](ll i,ll j,ll k)
    {
        ll val=a[i][j];
        ll x=0;
        for(auto it=mp.lower_bound({val,j});it!=mp.upper_bound({val,j});++it)
        {
            if(k==0)
            {
            x=max(x,1+permute(i,j+1,(*it).second));
            }
            else if((*it).second==k)
            {
                x=max(x,1+permute(i,j+1,k));
            }
        }
        return x;
    };
