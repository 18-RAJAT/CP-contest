...
bool solve()
{
    ...
    //axis
    ll x1,y1,x2,y2;
    cin>>w>>h>>x1>>y1>>x2>>y2>>newW>>newH;

    auto ans=MOD;

    if(abs(x2-x1+newW)<=w)
    {
        // ans=min(abs(w-newW-x2),abs(x1-newW));
        ans=min(ans,max(0,newW-x1));
        ans=min(ans,max(0,x2-(w-newW)));
    }
    if(abs(y2-y1+newH)<=h)
    {
        // ans=min(abs(h-newH-y2),abs(y1-newH));
        ans=min(ans,max(0,newH-y1));
        ans=min(ans,max(0,y2-(h-newH)));
    }
    if(ans==MOD)
    {
        cout<<-1<<nl;
