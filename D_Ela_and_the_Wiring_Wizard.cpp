...
{
    // ll n,m;cin>>n>>m;
    // VII distance(n,VI(n,INF)),distance1(n,VI(n,INF));
    ...
    //     distance1[u][v]=min(distance1[u][v],w);
    //     distance1[v][u]=min(distance1[v][u],w);
    // }
    // f1(k,0,n)
    // {
    //     f1(i,0,n)
    ...
    //     f1(j,0,n)
    //     {
    //         if(i!=j and distance1[i][j]!=INF)
    //         {
    //             res=min(res,(distance[i][0]+distance[j][n-1]+1)*distance1[i][j]);
    //             f1(k,0,n)
    //             {
    //                 res=min(res,(distance[0][k]+distance[n-1][k]+distance[i][k]+2)*distance1[i][j]);
    //             }
    //         }
    //     }
    // }
...
{
    cin>>u[i]>>v[i]>>w[i];
    dp[u[i]][v[i]]=min(dp[u[i]][v[i]],1);
    dp[v[i]][u[i]]=min(dp[v[i]][u[i]],1);
}
cf(k,1,n)
{
    cf(i,1,n)
...
cf(i,1,m)
{
    ans=min({ans,(1+dp[1][u[i]]+dp[v[i]][n])*w[i],(1+dp[1][v[i]]+dp[u[i]][n])*w[i]});
    cf(j,1,n)
    {
        ans=min({ans,(1+dp[1][j]+dp[j][n]+dp[u[i]][j]+1)*w[i],(1+dp[1][j]+dp[j][n]+dp[v[i]][j]+1)*w[i]});
    }
