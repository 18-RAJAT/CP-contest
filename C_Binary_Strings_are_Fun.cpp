...
void solve()
{
    ...
    ll compute[n];
    // memset(compute,1,sizeof(compute));
    ...
    f1(i,2,m)
    {
        if(s[i-1]==s[i])
        {
            //move to the 2*dp[i-1] position and add 1 to it

            //ways to track the ith position with the same character as the (i-1)th position
            dp[i]=dp[i-1]*2%MOD;
            // compute[i]=compute[i-1]+1;

            //computing the number of ways to reach the ith position
            // dp[i]=dp[i]%MOD;
        ...
        else
        {
            ...
            //compute and add the number of ways to reach the ith position
            // dp[i]=compute[i-1];
            //i-1ith position is the last position of the substring
            // compute[i]=compute[i-1];
