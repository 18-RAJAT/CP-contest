...
bool ok(int x,int y)
{
    return (min(x,y)>=0 and x<n and y<m and c[x][y]!='*');
}
...
void solve()
{
    function<int(int,int)>dist=[&](int i,int j)->int
    {
        return abs(i-xx)+abs(j-yy);
    };
    ...
    function<void(int,int)>dfs=[&](int i,int j)->void
    {
        ...
        if(ok(i+1,j) and dist(i+1,j)<=k-1)
        {
            s+='D';
            --k;
            dfs(i+1,j);
        }
        else if(ok(i,j-1) and dist(i,j-1)<=k-1)
        {
            s+='L';
            --k;
            dfs(i,j-1);
        ...
        else if(ok(i-1,j) and dist(i-1,j)<=k-1)
        {
            s+='U';
            --k;
            dfs(i-1,j);
