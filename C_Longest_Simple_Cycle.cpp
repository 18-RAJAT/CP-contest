...
void solve()
{
    ...
    f1(j,1,n)
    {
        ...
        else
        {
            ...
            if(j==1) //or j==n-1)
            {
                previous+=abs(vb[j]-va[j]);
                continue;
            }
            ll mini=min(va[j],vb[j]);
            ll maxi=max(va[j],vb[j]);

            ll db=abs(vc[j-1]-maxi)+abs(mini-1);

            // int diff=abs(vc[j]-vc[j-1]);
            ll diff=vc[j]-1;

            ans=max(ans,previous+diff);
            previous+=db;
        }
        // int valid=db;
        // if(j==n-1)
        // {
        //     previous+=abs(va[j]-vb[j]);
        //     previous+=2;
        //     previous+=vc[j];
        //     previous-=1;
        //     ans=max(ans,previous);
