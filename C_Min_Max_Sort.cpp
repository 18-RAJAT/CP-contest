...
int bs(int l,int r,auto &&tp)//function<int(int)>f)
{
    while(l<r)
    {
        int mid=(l+r)/2;
...
void solve()
{
    ...
    // function<int(int,int,auto)>bs=[&](int l,int r,auto bs)->int
    // {
    //     if(l>r)return -1;
    //     int mid=(l+r)/2;
    //     if(v[mid]==mid)return mid;
    //     if(v[mid]>mid)return bs(l,mid-1,bs);
    //     return bs(mid+1,r,bs);
    // };
