...
void solve()
{
    ...
    auto MathOperation=[&]()->void
    {
        //1st is already calculated
        f1(i,2,n-1)
        {
            //to reach i from i-1 we have two options either we go from i-1 to i-2 and then i-2 to i or we go from i-1 to i directly
    ...
    //To reach n-1 from n-2 we have two options either we go from n-2 to n-3 and then n-3 to n-1 or we go from n-2 to n-1 directly

    ll ans=min(dp[n-2][0]+Y[n-2]*ar[n-1],dp[n-2][1]+X[n-2]*ar[n-1]);
    cout<<ans<<nl;
}
