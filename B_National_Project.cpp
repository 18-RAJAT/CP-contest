...
void solve()
{
    ...
    ll need,full,extra,maxAnswer;
    //possible scenarios

    //if we use all the g's then we need=(n+1)/2
    need=(n+1)/2;
    //if need==0 then we can't use any g's
    full=(need-1)/g;
    //if full==0 then we can't use any g's
    extra=need-full*g;
    //if extra==0 and extra>0 then we can use all the g's
    maxAnswer=full*(g+b)+extra;

    debug(need,full,extra);
    
    cout<<(maxAnswer=max(maxAnswer,n))<<nl;
}
};
