...
void solve()
{
    function<int(vector<vector<int>>,int,int)>dfs=[&](vector<vector<int>>graph,int u,int v)->int
    {
        vector<vector<int>>newGraph=graph;
        newGraph[u].INS(newGraph[u].end(),newGraph[v].begin(),newGraph[v].end());
        sort(newGraph[u].begin(),newGraph[u].end());
        newGraph[u].erase(unique(newGraph[u].begin(),newGraph[u].end()),newGraph[u].end());
        newGraph[v].clear();
        f1(i,0,sza(newGraph))
        {
            auto it=find(newGraph[i].begin(),newGraph[i].end(),v);
            if(it!=newGraph[i].end())
            {
                newGraph[i].erase(it);
            }
        }
        int res=0;
        for(auto row:newGraph)
        {
            res+=sza(row);
        }
        res/=2;
