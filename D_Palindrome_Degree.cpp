...
void solve()
{
    function<void(string)>manaCher=[&](string s)
    {
        int n=s.size();
        int lps[n];
        int c=0,r=0;
    ...
    f1(i,0,n)
    {
        if(i<r)
        {
            lps[i]=min(lps[i-1],r-i);
        }
        while(i+lps[i]<n and str[lps[i]]==str[i+lps[i]])
        {
            lps[i]++;
        }
        if(i+lps[i]-1>r)
    ...
    f1(i,0,n)
    {
        ...
        if(lps[n-i-1]==1+i)
        {
            used[i]=1+(i?used[i/2+1]:0);
        }   
    }
    int acc=accumulate(used,used+n,0);
    // cout<<acc-1<<nl;
}
