...
void solve()
{
	...
	VI subArray(n+2,0),max_subArray(n+2,0);
	ll mxi=0;
	...
	for(ll i=n;i>=1;--i)
	{
		auto used_sub=[&]()->void
		{
			max_subArray[i]=max(max_subArray[i],1+max_subArray[i+store+1]);
		};
		max_subArray[i]=1+mxi;
		...
		{
			used_sub();
			subArray[i]=1+subArray[i+tmp+1];
		}
		mxi=max<ll>(mxi,subArray[i]);
	}
    VI ert(n+1,2);
	rf(i,n,1)
	{
		// if(i==n-1)
		// {
		// 	int mn=min<int>(ert[i],1);
		// 	ert[i]=mn;
		// }
		if(subArray[i+1]==a[i])
		{
			ll mn=min<ll>(ert[i],0);
			ert[i]=mn;
		}
		if(subArray[i+1]>0)
		{
			ll mn=min<ll>(ert[i],1);
			ert[i]=mn;
		}
		if(max_subArray[i+1]>=a[i])
		{
			ll mn=min<ll>(ert[i],1);
			ert[i]=mn;
		}
