...
void solve()
{
    ...
    string currPos="";//good/(empty)
    f1(i,0,n)
    {
        if(currPos.size()%2==0 or s[i]!=currPos.back())
        {
            //this is a good position
            currPos+=s[i];
        }
    }
        //if the size is odd then we can remove the last element
        if(currPos.size()%2)currPos.pop_back();
        cout<<n-currPos.size()<<nl;
        cout<<currPos<<nl;
}
