...
void solve()
{
    ...
    int src=0;
    int win[n];MEM(win,src);
    auto check=[&](int node)->bool
    {
        if(sza(adj[node])==1 and win[node]==1)
        {
            return true;
        }
        return false;
    ...
    function<void(int)>bfs=[&](int node)->void
    //node is the parent of the current node int the bfs tree
    {
        queue<int>bfs;
        int jo=-1;
        for(auto& it:ele)
